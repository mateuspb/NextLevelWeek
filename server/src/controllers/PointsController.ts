...
class PointsController{
    async index(request: Request, response: Response) {
        ...
        const points = await knex('points')
            .join('point_items', 'points.id', '=', 'point_items.id_points')
            .whereIn('point_items.id_items', parsedItems)
    ...
    async show (request: Request, response: Response) {
        ...
        const items = await knex('items')
            .join('point_items', 'items.id', '=', 'point_items.id_items')
            .where('point_items.id_points', id)
            .select('items.titulo');
